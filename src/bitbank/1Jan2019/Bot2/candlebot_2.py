...
def mod_data(data, mindif, maxdif):
    ...
    data["Difindex"] = 1 * (
        (mindif < data["End"] - data["Start"]) & (data["End"] - data["Start"] < maxdif)
    ) - 1 * (data["End"] - data["Start"] < 0)
    buylist = [0, 0]
    for i in range(2, len(data["Start"])):
        buylist.append(
            data.at[data.index[i], "Difindex"] + data.at[data.index[i - 1], "Difindex"]
        )
    ...
    data["Higeindex"] = 1 * (
        ((data["Max"] - data["End"]) / (data["End"] - data["Start"]) >= 0)
        & ((data["Max"] - data["End"]) / (data["End"] - data["Start"]) < 0.1)
    )
...
class Millions_bot:
    ...
    def calculate_profit(self, data):
        ...
        while i < len(data["Start"]):
            if self.staytime == 0:
                if (
                    self.position == 0 and data.at[data.index[i - 1], "Buyindex"] == 2
                ) and (
                    data.at[data.index[i - 1], "Higeindex"]
                    * data.at[data.index[i - 2], "Higeindex"]
                    == 1
                ):  # ポジション無しかつ上昇トレンド
                    self.position = 1
                    ...
                    if data.at[data.index[i], "Start"] > self.price:  # 買値より高ければ、売却して利確
                        ...
                        data.at[data.index[i], "Start"] < self.price
                    ):  # 買値より安い場合は、様子を見るか損切り
                        if self.remtime > 0:  # 待ち時間がある
                            if (
                                data.at[data.index[i], "Start"] - self.price < -200
                            ):  # 200円以上下がったら、すぐに損切り(急落と判断)
                                self.totprofit += (
                                    data.at[data.index[i], "Start"] - self.price
                                )
                                self.position = 0
...
def main():
    ...
    for date in datelist:
        ...
        # 取引データの読み込みと修正
        path = "/Users/takahisa/Desktop/Bot/1Jan2019/Candle_data/"
