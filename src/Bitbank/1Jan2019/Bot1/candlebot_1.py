...
def mod_data(data, mindif, maxdif):
    ...
    list = pd.DataFrame({"Buyindex": list}, index=data.index)
...
class Millions_bot:
    def __init__(self, time):
        ...
        self.profit_list = []  # トータルの利益のヒストリカル推移
            ...
            if self.staytime == 0:
                if (
                    ...
                    if data.at[data.index[i], "Start"] > self.price:  # 買値より高ければ、売却して利確
                        ...
                        if self.remtime > 0:  # 待ち時間はまだある
                            if (
                                data.at[data.index[i], "Start"] - self.price < -1000
                            ):  # 1000円以上下がったら損切り
                                self.totprofit += (
                                    data.at[data.index[i], "Mid"] - self.price
                                )
                                self.position = 0
                                self.remtime = self.time
                            else:  # 下降トレンドにはないと判断
