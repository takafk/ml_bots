...
def calculate_prediction(data):
    ...
    data['condition'] = data['histvol_prev'].rolling(time_prev).apply(judge_condtion,raw=True)
    ...
    if condition == 'reversion': # Mean reversion
        data['feature'] = -1 * data['close'].rolling(window=reversion_window)\
            .apply(lambda x:(x[-1] - x[0])/x[0], raw=True).shift(predict_min)
    else: # Momentum
        data['feature'] = data['return'].rolling(window=Momentum_window)\
        .mean().shift(predict_min)
    ...
    if bitmex.fetch_balance()['BTC']['total'] > lot:
        ...
        while error_times < ERROR_TIME:
            try:
                ...
                value = bitmex.create_order(pair, type='limit', side=side,\
                                            amount=size, price=price, params=params)
    ...
    if bitmex.fetch_balance()['BTC']['total'] > lot:
        ...
        while error_times < ERROR_TIME:
            try:
                ...
                value = bitmex.create_order(pair, type='StopLimit', side=side,\
                                            amount=size, price=price, params=params)
    ...
    if bitmex.fetch_balance()['BTC']['total'] > lot:
        ...
        while error_times < ERROR_TIME:
            try:
                ...
                value = bitmex.create_order(pair, type='stop', side=side,\
...
if __name__ == '__main__':
    ...
    while True:
        ...
        try:
            ...
            DELTA_CLEARPOS = 1 # Order price difference from last price, unit is $, iniital is 10$
            DELTA_HAVPOS = 0.5 # Order price difference from last price, unit is $, iniital is 10$
            ...
            if position_size > 0: # Long position
                ...
                if last_price >= position_price + DELTA_CLEARPOS: # Profitable
                    ...
                    if (int(position_price) - TRIGGER) > last_price:
                        order_stop_market(PAIR, LOT, 'sell', abs(position_size), int(position_price),\
                        TRIGGER)
                        sleep(WAITING_TIME)
                    else:
                        order_limit(PAIR, LOT, 'sell', abs(position_size), int(position_price), DELTA)
                        order_stop_limit(PAIR, LOT, 'sell', abs(position_size), int(position_price),\
                        TRIGGER)
                        sleep(WAITING_TIME)
                ...
                if last_price <= position_price- DELTA_CLEARPOS: # Profitable
                    ...
                    if (int(position_price) + TRIGGER) < last_price: # Immediately clear position
                        order_stop_market(PAIR, LOT, 'buy', abs(position_size), int(position_price), \
                        TRIGGER)
                        sleep(WAITING_TIME)
                    else:
                        order_limit(PAIR, LOT, 'buy', abs(position_size), int(position_price), DELTA)
                        order_stop_limit(PAIR, LOT, 'buy', abs(position_size), int(position_price), \
                        TRIGGER)
                        sleep(WAITING_TIME)
