...
def mod_data(data, mindif, maxdif):
    ...
    for i in range(2,len(data['Start'])):
        list.append(data.at[data.index[i],'Difindex'] + data.at[data.index[i-1],'Difindex'] )
...
class Millions_bot:
    ...
    def calculate_profit(self, data):
        ...
        while i < len(data['Start']):
            if self.staytime == 0:
                if (self.position == 0 and data.at[data.index[i-1], 'Buyindex'] == 2): #ポジション無しかつ上昇トレンド
                    self.position = 1
                    ...
                    if data.at[data.index[i], 'Start'] > self.price: # 買値より高ければ、売却して利確
                        ...
                        if self.remtime > 0: # 待ち時間はまだある
                            if data.at[data.index[i], 'Start'] - self.price < -1000: #1000円以上下がったら損切り
